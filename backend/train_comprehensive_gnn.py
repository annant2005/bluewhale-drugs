    ...
    for idx, row in df.iterrows():
        ...
        for endpoint in toxicity_endpoints:
            ...
            if pd.isna(value) or value == '':
                toxicity_labels.append(0.5)  # Neutral for missing data
            else:
...
def train_comprehensive_gnn(data_list, toxicity_endpoints, epochs=50, batch_size=32, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            if hasattr(batch, 'batch') and batch.batch is not None:
                ...
                y_reshaped = batch.y.view(-1, len(toxicity_endpoints))
                loss = F.binary_cross_entropy(out, y_reshaped)
            else:
                # Single sample
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                if hasattr(batch, 'batch') and batch.batch is not None:
                    ...
                    val_targets.extend(y_reshaped.cpu().numpy())
                else:
                    loss = F.binary_cross_entropy(out, batch.y)
                    val_predictions.append((out > 0.5).float().cpu().numpy())
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            if hasattr(batch, 'batch') and batch.batch is not None:
                ...
                final_targets.extend(y_reshaped.cpu().numpy())
            else:
                final_predictions.append(out.cpu().numpy())
                final_targets.append(batch.y.cpu().numpy())
...
if __name__ == "__main__":
    ...
    # Train model
    model, endpoints = train_comprehensive_gnn(data_list, toxicity_endpoints, epochs=50) 